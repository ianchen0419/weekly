...
// mobile
var mq = [], mobilemaxwidth = '(max-width: 680px)', mobileminwidth = '(min-width: 680px)';
mq.mobile = Modernizr.mq(mobilemaxwidth);
mq.pc = Modernizr.mq(mobileminwidth);
var navisub = document.getElementById('navi-sub'),
...
var mobileMenu = document.createElement('div'), mobileUl = document.createElement('ul'), mobileMenuClose = document.createElement('div'), wrap = document.createDocumentFragment(), mobileMenuBt = document.createElement('div');
...
function mobilenaviFunc() {
    var naviElm = document.createDocumentFragment(), currentPosition = window.scrollY;
    function naviMli(lielm) {
        for (var i = 0; i < lielm.length; i++) {
            var nsli = lielm[i].cloneNode(true);
    ...
    for (let i = 0; i < qaElm.length; i++) {
        ...
        for (let j = 0; j < qadl.length; j++) {
            ...
            qadt.addEventListener('click', function (ev) {
                var currentDd = ev.target.nextElementSibling;
...
function sliderfunc(slideelm) {
    if (Modernizr.flexbox) {
        var slideMain = document.getElementsByClassName('gallery-top')[0], slideElmClone = document.createDocumentFragment(), slideElm = slideMain.getElementsByClassName('swiper-slide'), slideThumbs = document.getElementsByClassName('gallery-thumbs')[0], slideThumbsWrap = slideThumbs.getElementsByClassName('swiper-wrapper')[0];
        for (var i = 0; i < slideElm.length; i++) {
            var clone = slideElm[i].cloneNode(true);
...
// END:TOPのslider
// reserved form 
var roomtable1 = document.getElementById('table-people-1num'), roomtable2 = document.getElementById('table-people-2num'), roomtable3 = document.getElementById('table-people-3num'), roomSElm = document.getElementById('propertydetails-select-table');
function roomSelect(roomSFuncElm) {
    var peopleNum1 = document.getElementById('people-number1'), peopleNum2 = document.getElementById('people-number2'), peopleNum3 = document.getElementById('people-number3'), peopleNum1Number = peopleNum1.textContent, peopleNum2Number = peopleNum2.textContent, peopleNum3Number = peopleNum3.textContent;
    // rowspanを設定
    var trNumber = roomtable1.getElementsByTagName('tbody')[0].getElementsByTagName('tr').length;
    ...
    var selectBox = document.createDocumentFragment(), selectWrap = roomSFuncElm.getElementsByClassName('propertydetails-select-table-people')[0], selectWrapUl = document.createElement('ul'), selectWrapLi1 = document.createElement('li'), selectWrapLi2 = document.createElement('li'), selectWrapLi3 = document.createElement('li');
    ...
    // 生成後
    var ulWidth = selectWrapUl.clientWidth, ulHeight = selectWrapLi1.clientHeight + 16 + 2, clickBox = document.createElement('div'), closeBox = document.createElement('div'), clickcloseBox = document.createDocumentFragment();
    ...
    if (mq.mobile) {
        var table1 = document.getElementById('table-people-1num'), table2 = document.getElementById('table-people-2num'), table3 = document.getElementById('table-people-3num');
        function mobileNumber(selectelm, peopleNumber) {
            var thead = selectelm.getElementsByTagName('thead')[0], trElm = document.createElement('tr'), tdElm = document.createElement('td'), peopleText;
...
// formへの引き渡し
var toFormWrap = document.getElementsByClassName('propertydetails-wrap')[0];
function toForm() { }
;


if (toFormWrap !== undefined) {
    var residentName, //物件名
    numberPeople, // 人数
    roomType, // 部屋タイプ
    plan, // プラン
    ...
    function peopleAddClass(peopleElm, classNames) {
        for (var i = 0; i < peopleElm.getElementsByClassName('to-reservation').length; i++) {
    ...
    // 人数をセットして実行
    typeFunc(people1, '1 Person');
    typeFunc(people2, '2 Persons');
    typeFunc(people3, '3 Persons');
    
    for (var i = 0; i < toReservation.length; i++) {
        
        toReservation[i].addEventListener('mouseover', function (ev) {
            
            var residentName = document.getElementsByClassName('propertydetails-category')[0].getElementsByTagName('h3')[0];
            var residentNameText = residentName.innerText; //物件名テキスト
            var thisElm = ev.target;
            var roomTypeText = thisElm.getAttribute('data-roomtype');
            var peopleNumberText = thisElm.getAttribute('data-people');
            var planText = thisElm.getAttribute('data-plan');
...
var reservationform = document.getElementsByClassName('reservationform')[0];

if (reservationform !== undefined) {
    
    var residentNameElm = document.getElementsByClassName('residentName-text')[0],
        residentNameText = document.createTextNode(decodeURIComponent(Cookies.get('weeklycenter_residentName'))),
        peoplenumberElm = document.getElementsByClassName('peoplenumber-text')[0],
        peoplenumberText = document.createTextNode(decodeURIComponent(Cookies.get('weeklycenter_people'))),
        roomTypeElm = document.getElementsByClassName('roomType-text')[0],
        roomTypeText = document.createTextNode(decodeURIComponent(Cookies.get('weeklycenter_roomType'))),
        ...
        residentNamevalue = decodeURIComponent(Cookies.get('weeklycenter_residentName')),
        peoplenumbervalue = decodeURIComponent(Cookies.get('weeklycenter_people'))
        roomTypevalue = decodeURIComponent(Cookies.get('weeklycenter_roomType'));
        planTypevalue = decodeURIComponent(Cookies.get('plan-text'));
    // 物件名
    console.log(Cookies.get('weeklycenter_people'))
    if (Cookies.get('weeklycenter_residentName') !== undefined) {
        ...
        residentNameElm.getElementsByTagName('input')[0].value = residentNamevalue;
    ...
    if (Cookies.get('weeklycenter_people') !== undefined) {
        ...
        peoplenumberElm.getElementsByTagName('input')[0].setAttribute('value', peoplenumbervalue);
    ...
    if (Cookies.get('weeklycenter_roomType') !== undefined) {
        roomTypeElm.getElementsByTagName('input')[0].setAttribute('value', roomTypevalue);
    ...
    if (Cookies.get('weeklycenter_plan') !== undefined) {
        if (Cookies.get('weeklycenter_plan') === 'short') {
            planElm.getElementsByTagName('input')[0].value = 'Short (7 ~ 29 nights)';
            if (confpage.length === 0) {
                // 確認画面では出さない
                planElm.getElementsByTagName('dl')[0].getElementsByTagName('dd')[0].appendChild(document.createTextNode('Short (7 ~ 29 nights)'));
            }
            ...
            planElm.getElementsByTagName('input')[0].value = 'Middle (30 ~ 89 nights)';
            if (confpage.length === 0) {
                // 確認画面では出さない
                planElm.getElementsByTagName('dl')[0].getElementsByTagName('dd')[0].appendChild(document.createTextNode('Middle (30 ~ 89 nights)'));
            }
        }
        else if (Cookies.get('weeklycenter_plan') === 'long') {
            planElm.getElementsByTagName('input')[0].value = 'Long (90 nights ~)';
            if (confpage.length === 0) {
                // 確認画面では出さない
                planElm.getElementsByTagName('dl')[0].getElementsByTagName('dd')[0].appendChild(document.createTextNode('Long (90 nights ~)'));
            }
        }
    ...
    if (resetBox.length !== 0) {
        function formreset(resetBt) {
            var resetButton = resetBt[0].getElementsByTagName('p')[0];
...
window.onload = function () {
    ...
    var sidePageTopBt = document.getElementById('page-side-banner'), pagetoppElm = document.getElementById('pageside-top');
    ...
    function linkwrap(linkBoxWrapelm) {
        if (linkBoxWrapelm.length != 0) {
            ...
            for (var i = 0; i < boxNum; i++) {
                ...
                if (linkhref != null) {
                    var linkLi = linkBoxWrapelm[i].parentNode, ahref = linkhref.getAttribute('href');
    ...
    linkwrap(linkboxtxt);
    // TOPの物件リストのアニメーション
    var toplist = document.getElementsByClassName('estate-list');
    function toplistFunc(listElm) {
        for (var i = 0; i < listElm.length; i++) {
            var li = listElm[i].getElementsByTagName('li');
            for (var u = 0; u < li.length; u++) {
                li[u].addEventListener('mouseover', function (ev) {
                    var textbox = li[u].getElementsByClassName('estate-list-text')[0], twtl = new TimelineMax();
                    twtl.to(li[u], 0.2, {
