...
const mainBlock:HTMLElement = document.getElementById('main'),
	  pagetitle:HTMLElement = document.getElementById('title');



// mobile
const mq:any = [],
	  mobilemaxwidth = '(max-width: 680px)',
	  mobileminwidth = '(min-width: 680px)';
mq.mobile = Modernizr.mq(mobilemaxwidth);
mq.pc = Modernizr.mq(mobileminwidth);
let navisub = document.getElementById('navi-sub'),
	navisubElm = navisub.getElementsByTagName('li'),
	navimain = document.getElementById('navi-main'),
	navimainElm = navimain.getElementsByTagName('li'),
	naviwrap = document.getElementById('header-navi');

let mobileMenu = document.createElement('div'),
	mobileUl = document.createElement('ul'),
	mobileMenuClose = document.createElement('div'),
	wrap = document.createDocumentFragment(),
	mobileMenuBt = document.createElement('div');
...
function mobilenaviFunc(){
	let naviElm = document.createDocumentFragment(),
		currentPosition = window.scrollY;
	function naviMli(lielm:any){
...
function qaFunc(qaElm:HTMLCollection){
	for(let i = 0; i < qaElm.length; i++){
		let qa = qaElm[i],
			qadl = qa.getElementsByTagName('dl'),
			timeLine = new TimelineMax();

		for(let j = 0; j < qadl.length; j++){

			let qadt:any = qadl[j].querySelectorAll('dt')[0],
				qadd = qadl[j].getElementsByTagName('dd')[0],
				arrowimg = qadl[j].getElementsByTagName('p')[0],
				closeBt = qadd.getElementsByClassName('arrow')[0];
...
// 実行処理内容
function sliderfunc(slideelm:any){
	if(Modernizr.flexbox){
		let slideMain = document.getElementsByClassName('gallery-top')[0],
			slideElmClone = document.createDocumentFragment(),
			slideElm = slideMain.getElementsByClassName('swiper-slide'),
			slideThumbs = document.getElementsByClassName('gallery-thumbs')[0],
			slideThumbsWrap = slideThumbs.getElementsByClassName('swiper-wrapper')[0];
...
// reserved form 
let roomtable1 = document.getElementById('table-people-1num'),
	roomtable2 = document.getElementById('table-people-2num'),
	roomtable3 = document.getElementById('table-people-3num'),
	roomSElm = document.getElementById('propertydetails-select-table');

function roomSelect(roomSFuncElm:any){
	let peopleNum1:HTMLElement = document.getElementById('people-number1'),
		peopleNum2:HTMLElement = document.getElementById('people-number2'),
		peopleNum3:HTMLElement = document.getElementById('people-number3'),
		peopleNum1Number = peopleNum1.textContent,
		peopleNum2Number = peopleNum2.textContent,
		peopleNum3Number = peopleNum3.textContent;
	// rowspanを設定
	...
	let selectBox = document.createDocumentFragment(),
		selectWrap = roomSFuncElm.getElementsByClassName('propertydetails-select-table-people')[0],
		selectWrapUl = document.createElement('ul'),
		selectWrapLi1 = document.createElement('li'),
		selectWrapLi2 = document.createElement('li'),
		selectWrapLi3 = document.createElement('li');
	...
	// 生成後
	let ulWidth = selectWrapUl.clientWidth,
		ulHeight = selectWrapLi1.clientHeight + 16 + 2,
		clickBox = document.createElement('div'),
		closeBox = document.createElement('div'),
		clickcloseBox = document.createDocumentFragment();
	...
	clickBox.addEventListener('click', function(ev:any){
	...
	closeBox.addEventListener('click', function(ev:any){
		TweenMax.to(clickBox, 0, {
			display: 'block'
		});
		TweenMax.to(selectWrapLi2, 0.3, {
	...
	if(mq.mobile){
		let table1 = document.getElementById('table-people-1num'),
			table2 = document.getElementById('table-people-2num'),
			table3 = document.getElementById('table-people-3num');
		function mobileNumber(selectelm:any, peopleNumber:any){
			let thead = selectelm.getElementsByTagName('thead')[0],
...
if(toFormWrap !== undefined){
	let residentName, //物件名
		...
		plan, // プラン
		squaresElm:any = [],
		toReservation = document.getElementsByClassName('to-reservation'),
		people1 = document.getElementById('table-people-1num'),
		people2 = document.getElementById('table-people-2num'),
		people3 = document.getElementById('table-people-3num');
	
	// classを人数毎のボタンに付与
	function peopleAddClass(peopleElm:any,classNames:any){
	...
	// 行毎に判定する
	function typeFunc(peopleBoxElm:any, pNumber:string){
		let people_tbody = peopleBoxElm.getElementsByTagName('tbody')[0],
			people_tr = people_tbody.getElementsByTagName('tr');
		for(let i = 0; i < people_tr.length; i++){
			...
			// roomtype set
			let p_short = people_tr[i].getElementsByClassName('td-short')[0].getElementsByClassName('to-reservation')[0].getElementsByTagName('a')[0],
				p_middle = people_tr[i].getElementsByClassName('td-middle')[0].getElementsByClassName('to-reservation')[0].getElementsByTagName('a')[0],
				p_long = people_tr[i].getElementsByClassName('td-long')[0].getElementsByClassName('to-reservation')[0].getElementsByTagName('a')[0];
	...
	for(let i = 0; i < toReservation.length; i++){
		toReservation[i].addEventListener('click', function(ev){
			...
			let residentNameText = residentName.innerText, //物件名テキスト
				thisElm = ev.target,
				roomTypeText = toReservation[i].getElementsByTagName('a')[0].getAttribute('data-roomtype'),
				peopleNumberText = toReservation[i].getElementsByTagName('a')[0].getAttribute('data-people'),
				planText = toReservation[i].getElementsByTagName('a')[0].getAttribute('data-plan');
...
if(reservationform !== undefined){
	let residentNameElm = document.getElementsByClassName('residentName-text')[0],
		residentNameText:any = document.createTextNode(decodeURIComponent(Cookies.get('weeklycenter_residentName'))),
		peoplenumberElm = document.getElementsByClassName('peoplenumber-text')[0],
		peoplenumberText:any = document.createTextNode(decodeURIComponent(Cookies.get('weeklycenter_people'))),
		roomTypeElm = document.getElementsByClassName('roomType-text')[0],
		roomTypeText:any = document.createTextNode(decodeURIComponent(Cookies.get('weeklycenter_roomType'))),
		planElm = document.getElementsByClassName('plan-text')[0],
		confpage = document.getElementsByClassName('formconf'),
		resetBox = document.getElementsByClassName('reservationreset');
	// 物件名
	...
	if(resetBox.length !== 0){
		function formreset(resetBt:any){
...
const scrollWrapBox:HTMLCollection = document.getElementsByClassName('sclb-cs');
function scrollbarfunc(scrollCustumWrap:any){
	let scrollboxNum:number = scrollCustumWrap.length;
...
let sidePageTopBt:HTMLElement = document.getElementById('page-side-banner'),
	pagetoppElm:HTMLElement = document.getElementById('pageside-top');
function sidepageTop(sidePageTopelm:HTMLElement){
...
if(mainBlock != null){
	let pagetopbox:NodeList = mainBlock.getElementsByClassName('pagetop');
	if(pagetopbox.length !== 0){

		function commonsidepageTop(pagetopelm:any){
			if(0 < pagetopelm.length){
					...
					for(var i = 0; i < pagetopboxNum; i++){
						pagetopelm[i].querySelector('a').addEventListener('click', function(event:any){
							event.preventDefault();
							TweenMax.to(window, 0.4, {scrollTo: 0});
						});
					}
...
function linkwrap(linkBoxWrapelm:any){
	if(linkBoxWrapelm.length != 0){
		...
		for(let i = 0; i < boxNum; i++){
			...
			if(linkhref != null){
				let linkLi:HTMLElement = linkBoxWrapelm[i].parentNode,
					ahref:string = linkhref.getAttribute('href');
				linkLi.addEventListener('mouseover', function(event:any){
					this.classList.add('onhover');
					this.style.cursor = 'pointer';
				});
				linkLi.addEventListener('mouseout', function(event:any){
					this.classList.toggle('onhover');
				});
				linkLi.addEventListener('click', function(event:any){
					let linkclass:string = this.querySelector('a').getAttribute('class'),
						linkBlank:string = this.querySelector('a').getAttribute('target');
...
function toplistFunc(listElm:any){
	for(let i = 0; i < listElm.length; i++){
		let li = listElm[i].getElementsByTagName('li');
		for(let u = 0; u < li.length; u++){
			li[u].addEventListener('mouseover', function(ev:any){
				let textbox = li[u].getElementsByClassName('estate-list-text')[0],
				twtl = new TimelineMax();
				twtl.to(li[u], 0.2, {
			...
			li[u].addEventListener('mouseout', function(ev:any){
				let textbox = li[u].getElementsByClassName('estate-list-text')[0],
				twtl = new TimelineMax();
				twtl.to(li[u], 0.1, {
		...
		$(function(){
			...
			$('#jquery-ui-datepicker-from').datepicker({
				...
				.on('changeDate', function(e){ //開始日が選択されたら
					...
					sd = computeDate(yyyy, mm, dd, 7); //0000-00-00の形で指定日後が返ってくる
