...
if (reservationform !== undefined) {
    ...
    if (Cookies.get('weeklycenter_plan') !== undefined) {
        if (Cookies.get('weeklycenter_plan') === 'short') {
            ...
            planElm.getElementsByTagName('input')[0].value = '长期（90晚〜）';
            if (confpage.length === 0) {
                // 確認画面では出さない
                planElm.getElementsByTagName('dl')[0].getElementsByTagName('dd')[0].appendChild(document.createTextNode('长期（90晚〜）'));
            }
        }
